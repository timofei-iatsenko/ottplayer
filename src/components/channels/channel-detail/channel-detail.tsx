  ...
  public render() {
    ...
    return (<div className={styles.details}>
      ...
      {epg.current && <div title={'Сейчас: ' + epg.current.name}
                   className={styles.currentProgram}>{epg.current.name}</div>}
      {epg.current && <ProgressBar startTime={epg.current.startTime} endTime={epg.current.endTime}/>}
      {epg.next &&
      <div title={'Затем: ' + epg.next.name} className={styles.nextProgram}>Затем: {epg.next.name}</div>}
    </div>);
  }
}
